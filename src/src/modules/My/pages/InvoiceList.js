          ...
          <div className={styles.invoice_list}>
            {
              dataset.length > 0 ? dataset.map((item, key) => {
                  return <CourseItem key={key} hasCheck onClick={() => this.handlerSelect(item.myId)} dataset={item}
                                     hideInfo />
                })
          ...
          <div className={styles.footer}>
            ...
            <div className={styles.btn_wrapper}>
              ...
              <div className={styles.right}>
                <Button type='primary' className={styles.my_btn} onClick={() => this.handlerNext(selectItem, sum)}
                        disabled={sum < 500}>下一步</Button>
              </div>
            </div>
          </div>
        </div>
