...
export default {
  ...
  effects: {
    ...
    * checkLogin ({payload = {}}, {put, call, select}) {
      ...
      // }
      const result = yield API.get('/Login/checkLogin')
    ...
    * fetchBrowserHistory ({payload = {}}, {put, select}) {
      const {params = {}, cb} = payload
      const {size, current} = yield select(state => state.my.myBrowserHistory)
      const result = yield API.get('/User/getVideoBrowserHistory', {size, current, ...params})
      yield put({type: 'fetchBrowserHistory__', payload: result})
      cb && cb()
    },
  ...
  reducers: {
    ...
    fetchInvoiceOrder__ (state, {payload = {}}) {
      ...
      let transformData = dataset.map(
        (item) => { return {...item.goods_list[0], historyTime: item.update_time, checked: false, myId: item.id, is_free: 0, money: item.money, order_type: item.order_type} })
